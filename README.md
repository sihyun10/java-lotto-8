...
구입한 로또 번호를 당첨번호와 보너스 번호를 비교해 당첨 결과 및 수익률을 계산해준다.

---

이 시스템에서 핵심 기능💡은 "로또 번호를 생성하고, 당첨 번호 및 보너스 번호와 비교하여 결과를 계산하는 것"입니다.

- 로또 번호 생성기 (LottoNumberGenerator)
    - 1~45 사이의 중복되지 않는 6개 숫자를 무작위로 생성한다.
    - 생성된 번호는 **오름차순으로 정렬**된다.
- 로또 한 장 (Lotto)
    - 로또 한 장의 번호(6개)를 표현하고 자신이 유효한지 검증한다.
        - 6개 숫자여야 함
        - 각 번호는 1~45 범위 내에 있어야 함
        - 중복된 번호가 없어야 함
        - 오름차순 정렬 상태여야 함
- 결과 계산기 (LottoResultCalculator)
    - 발행된 로또를 당첨 번호 및 보너스 번호와 비교한다.
    - 일치 개수 및 보너스 번호 일치 여부에 따라 등수를 판별한다.
